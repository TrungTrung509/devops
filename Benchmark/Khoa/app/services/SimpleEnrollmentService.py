...
class SimpleEnrollmentService:
        ...
        try:
            ...
            try:
                ...
                if transfers:
                    ...
                    with ThreadPoolExecutor(max_workers=min(len(transfers), 5)) as ex:
                ...
                CONFLICT_CHECK_DURATION.labels(app="khoa").observe(time.perf_counter() - t0)
            ...
            DB_ERRORS_TOTAL.labels(type=label, app="khoa").inc()
    ...
    def _do_enroll(user_id, home_site, ma_lop_hp, target_site):
        ...
        try:
            ...
            LOCK_HOLD_DURATION.labels(type="enroll", app="khoa").observe(time.perf_counter() - t0)
            return {"status": "Success", "type": "new", "message": f"Đăng ký ({home_site} → {target_site})"}
        except Exception:
            for db in dbs.values():
                db.rollback()
            LOCK_HOLD_DURATION.labels(type="enroll_fail", app="khoa").observe(time.perf_counter() - t0)
            raise
        finally:
            for db in dbs.values():
                db.close()
    ...
    def _swap_with_retry(user_id, home_site, old_ma_lop_hp, old_site, new_ma_lop_hp, new_site):
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                is_retryable = ((isinstance(e, HTTPException) and e.status_code == 503) or "deadlock" in str(e).lower() or "lock timeout" in str(e).lower())
                if not is_retryable:
                    raise
                last_exc = e
    ...
    def _do_swap(user_id, home_site, old_ma_lop_hp, old_site, new_ma_lop_hp, new_site):
        ...
        try:
            ...
            LOCK_HOLD_DURATION.labels(type="swap", app="khoa").observe(time.perf_counter() - t0)
            return {"status": "Success", "type": "swap", "message": f"Đổi lớp {old_ma_lop_hp} → {new_ma_lop_hp}"}
        except Exception:
            for db in dbs.values():
                db.rollback()
            LOCK_HOLD_DURATION.labels(type="swap_fail", app="khoa").observe(time.perf_counter() - t0)
            raise
        finally:
            for db in dbs.values():
                db.close()
    ...
    def cancel_simple(user_id: str, ma_lop_hp: str):
        ...
        try:
            ...
            LOCK_HOLD_DURATION.labels(type="cancel", app="khoa").observe(time.perf_counter() - t0)
            ...
            for db in dbs.values():
                db.rollback()
            LOCK_HOLD_DURATION.labels(type="cancel_fail", app="khoa").observe(time.perf_counter() - t0)
            raise
        finally:
            for db in dbs.values():
                db.close()
